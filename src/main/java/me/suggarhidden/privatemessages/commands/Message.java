...
public class Message implements SimpleCommand {
  ...
  public void execute(SimpleCommand.Invocation invocation) {
    ...
    if (args.length < 2) {
      source.sendMessage(
              MiniMessage.miniMessage().deserialize(PrivateMessages.getConfig().getString("messages.message_usage")));
      return;
    }

    if (args[0].equalsIgnoreCase(sender.getUsername())) {
      sender.sendMessage(
              MiniMessage.miniMessage().deserialize(PrivateMessages.getConfig().getString("messages.message_self")));
      return;
    }
    ...
    if (targetOpt.isEmpty()) {
      source.sendMessage(MiniMessage.miniMessage()
              .deserialize(PrivateMessages.getConfig().getString("messages.player_not_found")));
      return;
    }
    ...
    sender.sendMessage(MiniMessage.miniMessage().deserialize(PrivateMessages.getConfig().getString("format.sender"),
            Placeholder.parsed("sender", sender.getUsername()), Placeholder.parsed("receiver", target.getUsername()),
            Placeholder.unparsed("message", msg)));

    target.sendMessage(MiniMessage.miniMessage().deserialize(PrivateMessages.getConfig().getString("format.receiver"),
            Placeholder.parsed("sender", sender.getUsername()), Placeholder.parsed("receiver", target.getUsername()),
            Placeholder.unparsed("message", msg)));

    PrivateMessages.getRecentMessages().put(sender.getUniqueId(), target.getUniqueId());
    if (PrivateMessages.getConfig().getBoolean("last_message_reply")) PrivateMessages.getRecentMessages().put(target.getUniqueId(), sender.getUniqueId());
  }
